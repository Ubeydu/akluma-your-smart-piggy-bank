...
{{--                        <h2 class="text-lg font-medium text-gray-900 mb-4">{{ __('Savings Plan') }}</h2>--}}
    ...
    : Carbon\Carbon::parse($summary['pick_date_step3']['date'])->utc()->setTimezone(config('app.timezone'))->locale(App::getLocale())->isoFormat('LL') }}</p>
                    ...
                    <div class="flex flex-col sm:flex-row sm:justify-between items-center gap-4 @auth mt-8 @endauth">
                        ...
                        <!-- Right side: Previous + action buttons -->
                        <div class="flex flex-col sm:flex-row items-center gap-3">
                            <x-secondary-button type="button" class="w-[200px] sm:w-auto justify-center" onclick="window.location='{{ localizedRoute('localized.create-piggy-bank.pick-date.step-3') }}'">
                                {{ __('Previous') }}
                            </x-secondary-button>
